...
class ItemizedDirectedModel extends MutableDirectedModel {
  ...
  def factors(variables: Iterable[Var]): Iterable[Factor] = {
    val result = new collection.mutable.HashSet[Factor]; variables.foreach(v => addFactors(v, result)); result
  }

  override def factors(v: Var): Iterable[Factor] = {
    val result = new collection.mutable.HashSet[Factor]; addFactors(v, result); result
  }
