...
object Beta extends DirectedFamily3[DoubleVariable, DoubleVariable, DoubleVariable] {
  self =>
  ...
  def variance(alpha: Double, beta: Double): Double = {
    val sum = alpha + beta; alpha * beta / (sum * sum * (sum + 1))
  }
