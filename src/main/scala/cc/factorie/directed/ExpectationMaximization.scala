...
class EMInferencer[V <: Var, W <: DiscreteVar, M <: Model](val maximizing: Iterable[V], val marginalizing: Iterable[W], val model: M, val expecter: Infer[Iterable[W], M], val maximizer: Infer[Iterable[V], M]) {
  ...
  def mStep(): Unit = maximizing.foreach(v => maximizer.infer(Seq(v), model, summary).setToMaximize(null))

  // This "get" will fail if the Maximizer was unable to handle the request
  def process(iterations: Int): Unit = for (i <- 0 until iterations) {
    eStep; mStep
  }

  // TODO Which should come first?  mStep or eStep?
